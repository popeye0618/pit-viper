...
| `style.py` | 이 프로젝트의 테스트 스타일 규칙 (`viper/test-style.md`) |
...
**먼저 이 프로젝트의 테스트 스타일 규칙을 읽는다.**

```bash
python3 .claude/skills/pit-viper/scripts/style.py
```

`viper/test-style.md` 를 출력한다. 없으면 기본값으로 만들어 준다.
**거기 적힌 규칙이 스타일에 관한 최종 권한이다** — 명명·구조·단언 방식을 임의로 정하지 않는다.
규칙과 어긋나는 기존 테스트가 있어도 새 테스트는 규칙을 따른다.

그리고 해당 소스 파일의 그 줄을 **읽는다.** 뮤테이터 이름만 보고 쓰지 않는다.
"이 변형을 넣었을 때 실패하는 테스트"를 쓴다 — 유형별 전략은 아래에 있다.
테스트는 `src/test` 의 대응 위치에 쓴다.
...
거부 케이스(`Limited(-1)` → 예외)만 쓰기 쉽다. 하지만 **합법 경계값(`Limited(0)`) 이 정상적으로 만들어지는지**도 함께 써야 한다.
그것이 없으면 가드가 `max < 0` 에서 `max <= 0` 으로 바뀌어 **합법 값 0 이 죽는 회귀**가 나도 아무 테스트가 깨지지 않는다.

`if (x < N) throw` 를 보면 **거부(N-1)와 수용(N) 을 쌍으로** 만든다. 예외 경로도 경계다.

### 기대값을 구현으로 계산하지 않는다

```java
// 나쁨 — 기대값을 대상과 같은 API 로 만들었다
assertThat(new Retention.Months(3).cutoffFrom(NOW)).isEqualTo(NOW.minusMonths(3));

// 좋음 — 평가된 리터럴로 굳혔다
assertThat(new Retention.Months(3).cutoffFrom(NOW)).isEqualTo(LocalDateTime.of(2026, 5, 16, 12, 0));
```

앞엣것은 단위 바꿔치기 변이는 잡지만 **`minusMonths` 의 의미를 잘못 이해했을 때 테스트와 구현이 같이 틀린다.**
명세를 고정하는 테스트가 아니라 현재 동작을 베끼는 특성화 테스트가 된다.

기대값은 **대상 메서드가 내부에서 쓰는 API 로 계산하지 않는다.** 손으로 계산한 값을 리터럴로 적는다.

## 목표 목록에 뜨지 않는 것

**뮤테이션은 "있는 로직을 비틀었을 때 테스트가 알아채는가"만 잰다.** 다음은 스코어가 100% 여도 검증되지 않는다.
목표 목록에 없다고 안전한 것이 아니므로, 아래 셋은 **스스로 챙긴다.**

1. **생성자.** pitest 는 `<init>` 에 뮤턴트를 만들지 않는다 (이 스킬로 실측 확인).
   컴팩트 생성자·팩토리의 검증 로직은 신호가 아예 도달하지 않는 사각지대다.
   위의 "가드는 거부와 수용을 쌍으로" 규칙이 여기서 특히 중요하다.
2. **데이터.** enum 상수에 꽂힌 정책 값, 설정 기본값은 비틀 코드가 없다.
   `BASIC` 요금제에 `Limited(0)` 대신 `Unlimited()` 를 꽂아도 스코어는 만점이다.
   → 적용값을 **리터럴로 고정하는 테스트**를 따로 쓴다.
3. **없는 코드.** 빠진 검증, 빠진 분기, 한 번도 만들어지지 않는 합법 경계값은 뮤턴트가 존재하지 않는다.
   "무엇이 빠졌는가"는 명세를 아는 사람만 안다.

`excludedClasses` 로 제외한 패키지(보통 `enums`·`dto`)도 측정 밖이다 — 2번과 겹치므로 특히 주의한다.
